
/* !!! This is code generated by Prisma. Do not edit directly. !!! */
/* eslint-disable */
// biome-ignore-all lint: generated file
// @ts-nocheck 
/*
 * This is a barrel export file for all models and their related types.
 *
 * 🟢 You can import this file directly.
 */
export type * from './models/Area.ts'
export type * from './models/Proceso.ts'
export type * from './models/Component.ts'
export type * from './models/Question.ts'
export type * from './models/QuestionOption.ts'
export type * from './models/Campaign.ts'
export type * from './models/SurveyResponse.ts'
export type * from './models/Answer.ts'
export type * from './models/IndicatorWeight.ts'
export type * from './models/IndicatorThreshold.ts'
export type * from './models/AdminAuditLog.ts'
export type * from './commonInputTypes.ts'